...
{
    // Exported types
    template <> _MINIMAL_EXPORT Type& type<Faction>() { static Type ty("Faction"); return ty; }
    
    template <> _MINIMAL_EXPORT Type& type<Player>() { static Type ty("Player"); return ty; }
    template <> _MINIMAL_EXPORT Type& type<Lamp>() { static Type ty("Lamp", type<mud::Complex>()); return ty; }
    template <> _MINIMAL_EXPORT Type& type<TileWorld>() { static Type ty("TileWorld", type<mud::Complex>()); return ty; }
